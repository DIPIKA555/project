...
export default function Tile(props) {
    ...
    const handleClick = () => {
        setSelected({ row: row, column: column, group: group, setValue: setValue, value: value, setCorrect: setCorrect })
    }

    return (
        <div className={current ? 'board-tile selected' : (inline ? 'board-tile inline' : 'board-tile')} onClick={handleClick}>
            <h4 className={!correct ? 'font-red' : !editable && same ? 'read-only font-accent-color' : same ? 'font-accent-color' : !editable ? 'read-only' : ''}>{value !== 0 ? value : ' '}</h4>
        </div>
    )
}